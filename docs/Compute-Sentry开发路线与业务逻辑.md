...
- [x] **任务 3.1**：开发 `Compute-Sentry-Agent` (DaemonSet)。负责接收各 Pod 发送的 UDS 数据包，并暴露 Prometheus Metrics 接口。 (`agent/main.go`, `collector/collector.go`, `manifests/agent-daemonset.yaml`)
- [x] **任务 3.2**：设计 **Grafana Dashboard 模板**。充分利用 PromQL 实现"型号-节点-Pod"的多维聚合。 (`agent/exporter/prometheus.go`)
