...
- [ ] **任务 1.1**：构建 C++ `Spy` 项目骨架，实现核心符号劫持（劫持 `ncclAllReduce`）。
- [ ] **任务 1.2**：设计无锁异步队列与 Unix Domain Socket (UDS) 传输协议。
- [ ] **任务 1.3**：编写 Mock 训练程序，验证劫持库能精准捕获延迟。
- [ ] **任务 1.4**：**Phase 1 Demo & Testing**：演示不改代码劫持 NCCL 调用并异步输出指标流。
- **交付物**：`libCompute-Sentry-Spy.so` 原型与验证报告。

### Phase 2: K8s 治理集成期 (Control Plane - Operator)
**目标**：实现 K8s 原生的自动注入与预检逻辑。
- [ ] **任务 2.1**：编写 `Compute-Sentry-Operator`。实现 Mutating Webhook，自动改写 Pod 环境变量（注入 `LD_PRELOAD`）。
- [ ] **任务 2.2**：实现 `InitContainer` 预检程序逻辑，集成算力指纹采集脚本。
- [ ] **任务 2.3**：设计 `Compute-SentryPolicy` CRD。允许用户针对不同任务设置不同的健康阈值（如：训练任务 1 可以忍受 5% 抖动，训练任务 2 必须 0 容忍）。
- [ ] **任务 2.4**：**Phase 2 Demo & Testing**：在 K8s 集群中演示 Pod 自动注入 Spy 库并成功启动。
